...
const ColorPaletteModel = ({ navigation }) => {
    const [colorPaletteName, setPaletteName] = useState("");
    ...
    const handleSubmit = useCallback(() => {
        if (!colorPaletteName) {
            ...
            Alert.alert('Please select minimum 3 colors')
        }
        else {
    ...
    }, [colorPaletteName, selectedColors]);
            ...
            <FlatList
                ...
                renderItem={({ item }) => (
                    <View style={styles.colors}>
                        <Text style={[{ color: item.hexCode }, styles.item]}> {item.colorName}</Text>
                        <Switch
